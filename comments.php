<?php
 ...
 //  Создаем свою callback фунцкию вывода каждого коммента - берем mytheme_comment с сайта для основы

	function universal_theme_comment( $comment, $args, $depth ) {
		// проверяем в каком стиле у нас родитель (ul ol div)
		if ( 'div' === $args['style'] ) {
			// если стиль - div то тего будет div
			$tag       = 'div';
			$add_below = 'comment';
		} else {
		...
		//  какие классы вешаем на каждый комментраий
			...
			if ( have_comments() ) :
				?>			
					...
					<ol class="comments-list">
						<?php
							// вывводим каждый отдельный комментарий
							//мы сами создадим ф-ю callback для вывода комментариев в самом начале файла
							wp_list_comments(
								array(
									'style'      => 'ol',
									'short_ping' => true,
